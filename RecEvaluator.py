...
class RecEvaluator:
    ...
    def _get_top_k_items(self, user: Union[int, str], k: int, module: Optional[str]) -> List[int]:
        """
        Given a user, return the top-k item IDs recommended. If multiple modules
        exist and `module` is not None, filter to that module. Otherwise, combine all modules.
        """
        # If we have multi-module data:
        if (
            ...
            if module is not None and module in self.user_recommendations[user]:
                # single module's recommendations
                return [
                    item
                ...
                # combine all modules for this user
                combined_items = []
                for mod, items_ranks in self.user_recommendations[user].items():
                    combined_items.extend([(item, rank) for item, rank in items_ranks if rank <= k])
                # Sort by rank again, just in case
